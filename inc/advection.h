...
void advectionz(complex<double>*****f,int nmax,int gridz,int gridx,int gridy,double dz,double v,double dt,char bcon)//A routine that advects one time step forward IN TOKAMAK GEOMETRY X=Z
{
  //Advection routine in z using a second order Runge-Kutta method  
  //The Points 0 and gridz are the ones that determine the Boundary conditions
  double A,B;
  int pos;
  int n,m,i,j;
  
  complex<double> **grad,**fint,**fave;
  grad = new complex<double>*[gridz+1];
  fint = new complex<double>*[gridz+1];
  fave = new complex<double>*[gridz+1];
  for(i=0;i<=gridz;i++)
    {
    ...
    for(j=0;j<gridy;j++)
      {
	    ...
	    for(n=m;n<nmax;n++)
	      {
		for(pos=1;pos<gridz;pos++)//Average over two spatial grid points
		  {
		...
		switch(bcon)
		  ...
		  case 'p'://Periodic boundary
		    fave[0][n] = 0.5*(f[i][j][0][n][m] + f[i][j][gridz-1][n][m]);
		    fave[gridz][n] = fave[0][n];
		    break;
		    ...
		    if(((m+n)%2)==1)
		      {
		      ...
		      }else
				{
			  		...
			  		fave[gridz][n]=f[i][j][gridz-1][n][m];
				}
		    break;
		  default:
		...
		//END OF BC
		for(pos=1;pos<gridz;pos++)//Calcualte the gradient across each spatial element
		  {
		...
		switch(bcon)
		  {
		  case 'p'://Periodic boundary
		    grad[0][n] = (f[i][j][0][n][m] - f[i][j][gridz-1][n][m])/dz;
		    grad[gridz][n] = grad[0][n];
		    break;
		  case 'r'://reflective boundary	 
		    if(((m+n)%2)==1)
		      {
			  grad[0][n] = 2.0*f[i][j][0][n][m]/dz;
			  grad[gridz][n] = -2.0*f[i][j][gridz-1][n][m]/dz;
		      }else
			{
			  grad[0][n] = 0.0;
			  grad[gridz][n] = 0.0;
	    ...
	    for(n=m;n<nmax;n++)
	    {	  	                 
			for(pos=0;pos<gridz;pos++)//Calcualte the gradient across each spatial element
		  	{
